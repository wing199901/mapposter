export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  const cached = await readEdgeBoundary(context.env, osmType, osmId)
  if (cached) {
  ...
  const upstream = await boundaryFromNominatim(osmType, osmId, context.env)
  if (!upstream.ok) {
  ...
  await writeEdgeBoundary(context.env, osmType, osmId, upstream.geometry)
